...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var AuthStrategy = require(options.module);
  if (options.strategy !== '') {
    AuthStrategy = AuthStrategy[options.strategy || 'Strategy'];
  }
  ...
  switch (authType) {
    ...
    default:
      passport.use(name, new AuthStrategy(_.defaults({
        ...
        (options.strategyCallback || function(x) {return x})(function(req, accessToken, refreshToken, profile, done) {
          if (link) {
            if (req.user) {
              self.userCredentialModel.link(
                req.user.id, name, authScheme, profile,
